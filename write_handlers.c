...
/************************* WRITE NUMBER *************************/
/**
 * write_number - Writes a signed integer to the buffer
...
/**
 * write_num - Writes a number to the buffer with padding and extra characters
 * @ind: Index at which the number starts in the buffer.
 * @buffer: Buffer array to handle print.
 * @flags: Calculates active flags.
 * @width: Width specifier.
	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ') /* Extra character to left of buffer */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
	...
	int flags, int width, int precision, int size)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS) /* Extra character to left of buffer [buffer>padd] */
		{
			return (write(1, &buffer[ind], length) + write(1, &buffer[0], i));
		}
		else /* Extra character to left of padding [padd>buffer] */
		{
